...
('debutant', 'Mots de passe', 'Quelle est la caractéristique d''un mot de passe robuste ?', 'Court et facile à retenir', 'Identique sur tous les comptes', 'Long, avec majuscules, chiffres et symboles', 'Le prénom de l''utilisateur', 3),
('debutant', 'Mots de passe', 'Faut-il utiliser le même mot de passe pour plusieurs sites ?', 'Oui, c''est plus simple', 'Non, chaque compte doit avoir un mot de passe unique', 'Seulement pour les réseaux sociaux', 'Cela n''a aucune importance', 2),
('debutant', 'Virus', 'Qu''est-ce qu''un virus informatique ?', 'Un programme qui améliore les performances du PC', 'Un programme malveillant capable de se propager et d''endommager un système', 'Un type de carte graphique', 'Un protocole réseau', 2),
('debutant', 'Virus', 'Quel outil permet de détecter et supprimer un virus ?', 'Un antivirus', 'Un navigateur web', 'Une imprimante', 'Un tableur', 1),
('debutant', 'Mises à jour', 'Pourquoi est-il important de mettre à jour régulièrement son système ?', 'Pour changer la couleur de l''interface', 'Pour corriger des failles de sécurité connues', 'Pour ralentir l''ordinateur', 'Ce n''est pas important', 2),
('debutant', 'Mises à jour', 'Que risque-t-on en ignorant les mises à jour de sécurité ?', 'Rien du tout', 'Une amélioration automatique des performances', 'Une exposition à des failles déjà corrigées par l''éditeur', 'Un gain de stockage', 3),
('debutant', 'Emails suspects', 'Quel est un signe typique d''un email de phishing ?', 'Une adresse d''expéditeur officielle et connue', 'Une urgence excessive et une demande de clic immédiat sur un lien', 'Un message sans aucune pièce jointe', 'Un email envoyé par un collègue habituel', 2),
('debutant', 'Emails suspects', 'Que faire si un email demande vos identifiants bancaires de toute urgence ?', 'Répondre immédiatement avec ses identifiants', 'Cliquer sur le lien pour vérifier', 'Ne pas répondre et signaler l''email comme suspect', 'Transférer l''email à tous ses contacts', 3),
('debutant', 'HTTPS', 'Que signifie le "S" dans HTTPS ?', 'Système', 'Sécurisé (Secure)', 'Serveur', 'Standard', 2),
('debutant', 'HTTPS', 'Quel élément visuel indique généralement une connexion HTTPS dans un navigateur ?', 'Un cadenas dans la barre d''adresse', 'Une icône de poubelle', 'Un drapeau', 'Un haut-parleur', 1);
