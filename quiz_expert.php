...
if (!niveauAccessible(NIVEAU, $niveauActuelUtilisateur)) {
    header('Location: quiz_intermediaire.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifierJetonCSRF($_POST['csrf_token'] ?? null)) {
    ...
    $resultatAffiche = [
        ...
        'reussi'          => $reussiPourCertificat,
        'detail'          => $detailReponses,
        'niveau_debloque' => $resultat['niveau_debloque'],
    ];
}

$questions = recupererQuestions($pdo, NIVEAU, 40);

$titrePage = 'Quiz Expert';
...
<div class="quiz-wrapper">
    ...
    <?php if ($resultatAffiche !== null): ?>
        ...
        <div style="display:flex; gap:14px; margin-top: 30px;">
            <a href="quiz_expert.php" class="btn btn-ghost btn-block">Refaire le quiz</a>
            <a href="<?= $resultatAffiche['reussi'] ? 'certificat.php' : 'profil.php' ?>" class="btn btn-primary btn-block">
                <?= $resultatAffiche['reussi'] ? 'Voir mon certificat' : 'Voir mon profil' ?>
            </a>
        </div>
        ...
        <div id="unlock-overlay" class="unlock-overlay">
            <div class="unlock-modal">
                ...
                <h2 class="unlock-title">Parcours terminé !</h2>
                <p class="unlock-subtitle">Félicitations ! Vous avez complété les 3 niveaux. Votre certificat est disponible.</p>
                <button class="btn btn-primary" onclick="fermerUnlock()">Voir mon certificat</button>
            </div>
        </div>
        <style>
        ...
        <script>
        function fermerUnlock(){window.location.href='certificat.php';}
    ...
    <?php else: ?>
        ...
        <form method="post" action="quiz_expert.php">
