 ...
 *
 * Le calcul du score est fait UNIQUEMENT en PHP (jamais en
 * JavaScript) : un utilisateur ne peut pas trafiquer son score
 * en modifiant le code de la page dans son navigateur.
 * ------------------------------------------------------------
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifierJetonCSRF($_POST['csrf_token'] ?? null)) {
    ...
    foreach ($idsQuestions as $idQuestion) {
        // On relit CHAQUE question + sa bonne réponse depuis la base
        // (jamais depuis le formulaire envoyé par le client, qui
        // pourrait avoir été modifié) pour vérifier la réponse donnée.
...
<div class="quiz-wrapper">
    ...
    <?php if ($resultatAffiche !== null): ?>
        ...
        <div class="result-summary">
            <span class="quiz-theme-tag">Niveau Débutant</span>
            ...
            <?php if ($resultatAffiche['reussi']): ?>
                <p style="color: var(--success); margin-top: 10px; font-weight: 600;">Niveau Intermédiaire débloqué !</p>
            <?php else: ?>
                <p style="color: var(--error); margin-top: 10px; font-weight: 600;">Seuil de 70% non atteint. Réessayez !</p>
            <?php endif; ?>
        </div>
        ...
        <div style="display:flex; gap:14px; margin-top: 30px;">
            <a href="quiz_debutant.php" class="btn btn-ghost btn-block">Refaire le quiz</a>
            <a href="<?= $resultatAffiche['reussi'] ? 'quiz_intermediaire.php' : 'profil.php' ?>" class="btn btn-primary btn-block">
        ...
        function fermerUnlock() {
            document.getElementById('unlock-overlay').style.animation = 'fadeInOverlay .2s ease reverse';
            setTimeout(() => document.getElementById('unlock-overlay').remove(), 200);
        }
    ...
    <?php else: ?>
        ...
        <div class="quiz-header">
            <span class="quiz-theme-tag">Niveau Débutant</span>
            <span><?= count($questions) ?> questions · seuil de réussite 70%</span>
        </div>
        <div class="progress-track"><div class="progress-fill" id="quizProgressFill" style="width: 0%;"></div></div>

        <form method="post" action="quiz_debutant.php">
